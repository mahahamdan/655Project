...
def checkSSLTLS(url): #takes a URL as input and checks various SSL/TLS features.
    try:
        ...
        if responseHTTPS.status_code == 200:#if the HTTPS response has a status code of 200, set the protocol to 'https'
           
            urlProtocol = 'https'
        
        else:
            print(f"The website {url} does not support HTTPS and is vulnerable to SSL stripping.")
            return

        #check HSTS and cookies
        if urlProtocol == 'https' and 'Strict-Transport-Security' in responseHTTPS.headers:
            print(f"The website {httpsURL} has HSTS enabled.")
        elif urlProtocol == 'https':
            print(f"The website {httpsURL} does not have HSTS enabled and might be vulnerable to SSL stripping.")#checks for Strict-Transport-Security in the header
        ...
        conn = sslContext.wrap_socket(socket.socket(socket.AF_INET), serverHostname=hostname) # Wrap the socket with the SSL context to create a secure socket.
        conn.connect((hostname, 443)) # Establish a connection to the server using the SSL-wrapped socket.
            
        
        print(f"Cipher suite: {conn.cipher()}")
        print(f"SSL/TLS version: {conn.version()}")
#--------------------------------------------------------------------------------------------------------------------------------------------------      
...
checkSSLTLS('www.site.idk')#need to still parse this
